...
/* Make a test star */
    ...
    make_object( 2, 0, "STAR", &star, &obj );
    for ( latitude = 0.0; latitude <= 90.0; latitude += 1.0 ) { 
...
//        printf ("JPL ephemeris DE%d open:\n  Beg. JD = %10.2f\n  End  JD = %10.2f\n\n", de_num, jd_beg, jd_end);
        ...
        obs_loc.where = 1;
        if ( error = place( jd_tt, &obj, &obs_loc, delta_t, 1, accuracy, &t_place )) {
...
//        equ2hor ( jd_ut1, delta_t, accuracy, 0.0, 0.0, &geo_loc, rat, dect, 0, &zd, &az, &rar, &decr );
        equ2hor ( jd_ut1, delta_t, accuracy, -0.0001, -0.0001, &geo_loc, t_place.ra, t_place.dec, 0, &zd, &az, &rar, &decr );
