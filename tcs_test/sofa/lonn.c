...
/* ICRS to observed. */
    for ( longitude = 0.0 ; longitude <= 180.0 ; longitude += 1.0 ) {
...
//  reprd ( "ICRS, epoch J2000.0:", rc, dc );
        printf ("%f\t%.9f\t%.9f\n", longitude, zob * DR2D,  aob * DR2D );

//      iauA2af( ndp, zob, &zd_sign, zd_dmsf );
//      iauA2af( ndp, aob, &az_sign, az_dmsf );
//      printf ("%f\t%c%3.3i\t%2.2i\t%2.2i.%*.*i\t", longitude, zd_sign, zd_dmsf[0], zd_dmsf[1], zd_dmsf[2], ndp, ndp, zd_dmsf[3] );
