...
//  Target object details in ICRS units
    ...
    double icrs_dec    =   90; // DEC [decimal]
...
//  Observer details +/- DD:MM:SS.sss
    ...
    double dob2; // DEC reverse conversion [radians]
    double rob2; // RA  reverse conversion [radians] 
...
printf("AZ=%f: ZD=%f\n",az, zd);
            ...
            if ( iauAtoc13 ( "A", az, zd, 
                             utc1, utc2, dut1,
                             obs_lon, obs_lat, obs_alt, cip_x, cip_y, obs_pres, obs_temp, obs_rh, obs_wl,
                             &rob2, &dob2 ) ) return -1;

//            printf ( "%i\t%i\t%.9f\t%.9f\n", lat_d, lon_d, (rob2 - rob)*DR2AS, (dob2 - dob)*DR2AS );
            printf ( "%.9f\t%.9f\n", (rob-rob2) * DR2AS, (dob-dob2)*DR2AS );
        }
        puts("");
    }
    return 0;
