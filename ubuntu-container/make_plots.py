...
plt.figure(1)
plt.clf()
plt.plot((data_docker[:,-2]-data_docker[:,1])/1e9, label="Time to run 100 times in Docker")
plt.plot([0,len(data_docker[:,1])],[np.mean((data_docker[:,-2]-data_docker[:,1])/1e9), np.mean((data_docker[:,-2]-data_docker[:,1])/1e9)], ls=":", color='C0')
plt.plot((data_host[:,-2]-data_host[:,1])/1e9, label="Time to run 100 times in host")
...
plt.figure(2)
plt.clf()
plt.plot((data_docker[:,1])/1e9, label="Time to run 1,000,000 times in Docker", color='C0')
plt.plot([0,len(data_docker[:,1])],[np.mean((data_docker[:,1])/1e9), np.mean((data_docker[:,1])/1e9)], ls=":", color='C0')
plt.plot((data_host[:,1])/1e9, label="Time to run 1,000,000 times in host", color='C1')
